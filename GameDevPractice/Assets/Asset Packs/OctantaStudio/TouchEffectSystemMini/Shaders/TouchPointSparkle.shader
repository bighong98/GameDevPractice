Shader "UI/TouchPointSparkle"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Optimized sparkle distance function using power function for concave diamond
            float sparkleSDF(float2 p) {
                float power = 0.5;
                float d = pow(abs(p.x), power) + pow(abs(p.y), power);
                return d * 0.42;
            }
            ...
            fixed4 frag (v2f i) : SV_Target {
                ...
                // === SPARKLE GEOMETRY DISTANCE CALCULATION ===
                // Sparkle uses power function to create concave diamond with curved edges
                float2 center = float2(0.5, 0.5);
                float2 p = (i.uv - center) * 2.0;
                float distance = sparkleSDF(p);
