    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target {
                ...
                // === SQUARE GEOMETRY DISTANCE CALCULATION ===
                // Square uses Chebyshev distance (max of coordinate absolute values)
                float2 center = float2(0.5, 0.5);
                float distance = max(abs(i.uv.x - center.x), abs(i.uv.y - center.y));
