...
namespace TH.Combat
{
    public readonly struct AttackSource // 무기, 스킬, 투사체, 장판 등 대미지를 발생시키는 모든 개별 공격의 정보 구조체
    {
        ...
        public AttackSource(IAttacker attacker, IGameStat attackSourceStat, float baseDamage)
        {
        ...
        // 스탯 기반 공격용 생성자
        public AttackSource(IAttacker attacker, IGameStat attackSourceStat) 
            : this(attacker, attackSourceStat, 0) { }

        // 비 스탯 기반 공격용 생성자 (고정 수치 데미지)
        public AttackSource(IAttacker attacker, float fixedDamage) 
            : this(attacker, null, fixedDamage) { }

        public HitRequest ToRequest(IDamageable target)
        {
            // return new HitRequest(Attacker, BaseDamage, target);
            return new HitRequest(Attacker, AttackSourceStat?.Value ?? BaseDamage, target);
        }
    }
}
