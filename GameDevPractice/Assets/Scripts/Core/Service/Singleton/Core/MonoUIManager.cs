...
namespace TH.UI
{
    public class MonoUIManager : MonoSingleton<MonoUIManager>
    {
        ...
        protected override void InitOnceAfterPreLoad(Scene scene)
        {
            base.InitOnceAfterPreLoad(scene);
        ...
        {
            Logg.Log($"[UIManager] InitAfterPreLoad]", Logg.LoggingMode.InProgress);
            SetSceneUIAsync(scene).ContinueWith(() =>
            {
        ...
        {
            ShowPopupUI<TH.UI.InventoryUI>("InventoryUI.prefab");
        }
        ...
        public T ShowPopupUI<T>(string uiName = null) where T : PopupUI
        {
                ...
                && IsPopupInStack<T>(out var inStackPopup))
            {
                ...
                switch (inStackPopup.DuplicatedPopupHandling)
                {
                    case PopupUI.DuplicatedPopupHandle.Replace: // 기존 닫고 새로 열기
                        Logg.Log($"[{nameof(MonoUIManager)}.{nameof(ShowPopupUI)}()] Replace mode: close existing popup and create new one", 
                                Logg.LoggingMode.Completed);
                    ...
                    case PopupUI.DuplicatedPopupHandle.Toggle: // 기존 닫기만 수행
                        ...
                        {
                            Logg.Log($"[{nameof(MonoUIManager)}.{nameof(ShowPopupUI)}()] Toggle mode: close existing popup without creating new one", 
                                    Logg.LoggingMode.Completed);
                            return null;
                        }
                        break;
            ...
            Logg.Log($"[{nameof(MonoUIManager)}.{nameof(ShowPopupUI)}()] new Popup. name: {popup.name} popupStack.Count: {popupStacks.Count}", 
                    Logg.LoggingMode.Completed);
            return popup;
        }
        ...
        public bool ClosePopupUI(PopupUI popup, bool escapableCheck = true, bool ignoreOpenThreshold = true, bool waitForAnimation = true)
        {
            ...
            {
                Logg.Log($"[{nameof(MonoUIManager)}.{nameof(ClosePopupUI)}()]: popup not found in stack : {popup.name}", Logg.LoggingMode.Completed);
                return false;
            }
        ...
        private void ClosePopupInternal(PopupUI popup, ObjectPool<IPoolObject> popupPool, bool waitForAnimation)
        {
            if (waitForAnimation)
            {
                ...
                popup.OnPopupClosedAsync().ContinueWith(() =>
                {
                    ...
                    catch (Exception e) { Logg.LogError($"[{nameof(MonoUIManager)}] Error during popup closing: {e}"); }
                }).Forget();
            }
            else
            {
